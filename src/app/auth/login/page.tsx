...
import { signInWithPassword } from "@/src/lib/supabase";
  ...
  return (
    <div className="mx-auto max-w-xl rounded-[2rem] bg-white p-10 shadow-sm dark:bg-slate-900">
      <div className="space-y-4">
        <p className="text-sm uppercase tracking-[0.25em] text-slate-500 dark:text-slate-400">로그인</p>
        <h1 className="text-3xl font-semibold text-slate-900 dark:text-slate-100">계정으로 로그인하세요</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">이메일과 비밀번호로 즉시 로그인할 수 있습니다.</p>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-3 text-sm text-slate-500 dark:text-slate-400 sm:flex-row sm:items-center sm:justify-between">
        <p>계정이 없으면 회원가입하거나 이름으로 아이디를 찾아보세요.</p>
        <div className="flex flex-wrap gap-3">
          <Link href="/auth/signup" className="rounded-3xl bg-slate-100 px-4 py-2 text-sm text-slate-700 transition hover:bg-slate-200 dark:bg-slate-950 dark:text-slate-200 dark:hover:bg-slate-800">
            회원가입
          </Link>
